...
resource "multicdn_preference_config" "advanced_preferences" {
  ...
  # Define region-specific minimum measurement count thresholds
  minimum_measurement_count_threshold = {
    world = 1000  # Global default minimum measurement count threshold
    
    continents = {
      "NA" = {
        default = 1100  # Higher threshold for North America
        countries = {
          "US" = 1200,  # Even higher for US
          "CA" = 1300   # Higher for Canada
        }
      },
      "EU" = {
        default = 1050  # Higher threshold for Europe
        countries = {
          "DE" = 1150,  # Higher for Germany
          "FR" = 1250   # Higher for France
        }
      },
      "AS" = {
        default = 900   # Lower threshold for Asia
      }
    }
  }
}
...
- `minimum_measurement_count_threshold` (Attributes) Minimum measurement count threshold configuration (see [below for nested schema](#nestedatt--minimum_measurement_count_threshold))
...
<a id="nestedatt--minimum_measurement_count_threshold"></a>
### Nested Schema for `minimum_measurement_count_threshold`

Optional:

- `continents` (Attributes Map) Continent-specific minimum-count thresholds (see [below for nested schema](#nestedatt--minimum_measurement_count_threshold--continents))
- `world` (Number) Global minimum-count threshold (> 0)

<a id="nestedatt--minimum_measurement_count_threshold--continents"></a>
### Nested Schema for `minimum_measurement_count_threshold.continents`

Required:

- `default` (Number) Default minimum-count threshold for the continent (>0)

Optional:

- `countries` (Map of Number) Country-specific minimum-count thresholds (>0)